...
// Dom Ready
    ...
    // Util
        ...
        var sidePanelWbTabs = $('#panel-div > .wb-tabs');
        var sidePanelTabList = sidePanelWbTabs.find(' > ul[role=tablist]');
        var sidePanelTabPanels = sidePanelWbTabs.find(' > .tabpanels');

        var mapContent = $('#mapContent');
        var loadIndicator = mapContent.find('#map-load-indicator');

        var subPanels = {};
        var subPanelLoadingAnimation;
        ...
        */
        var subPanelPrototype = {
            ...
            */
            update: function (a) {
                ...
                var updateDefered = [new Deferred(), new Deferred()];

                var animateContent = function (node, newData, d) {
                    if (newData) {
                ...
                var setContent = function (node, oldData, newData, parsedData, visible, d) {
                ...
                var updateContent = function (a) {
                        ...
                        TweenLite.to(_this._subPanelContentDiv, animateContentDuration / 1000,
                            { scrollTop: 0, ease: 'easeOutCirc' });

                        setContent(_this._panelTitle, _this._attr.title, a.title, a.title,
                            _this._visible, updateDefered[0]);
                        setContent(_this._panelContentDiv, _this._attr.content, a.content,
                            _this.parseContent(a.content), _this._visible, updateDefered[1]);

                        dojoLang.mixin(_this._attr, a);
                    };
        ...
        var helpSectionContainer = $('#help-section-container');
        var helpSection = $('#help-section');

        var cssButtonPressedClass = 'button-pressed';
        var cssExpandedClass = 'state-expanded';
        ...
        toolbarController = (function () {
            ...
            var toolsTimeline = new TimelineLite();

            var isTooltipsSet = false;
        ...
        */
        layoutController = (function () {
            ...
            var mapToolbar = $('#map-toolbar');
            var basemapControls = $('#basemapControls');

            var panelDiv = $('#panel-div');
            var panelToggle = $('#panel-toggle');
            var panelPopup;
            var panelWidthDefault; // default width of the SidePanel.
            var layoutWidthThreshold = 1200; // minimum width of the wide layout

            var dataTabNode;
            var dataTabUpdateNotice;

            var windowWidth;

            var layoutChange;

            var _isFullData = false;
            var fullDataTimeLine = new TimelineLite({
                    paused: true,
                    onComplete: function () {
                        adjustHeight();
                        layoutChange();
            ...
            var panelToggleTimeLine = new TimelineLite({ paused: true });
            var fullDataSubpanelChangeTimeLine = new TimelineLite({ paused: true });
            var noDataTimeLine = new TimelineLite({
                    paused: true,
                    onComplete: function () {
                        // kill key events to stop tab switching when the other tab is hidden
                        panelDiv.on('keydown', '.wb-tabs > ul > li > a', stopEventPropagation);
            ...
            var createFullDataTL;
            var createPanelToggleTL;
            var createFullDataSubpanelChangeTL;
        ...
        */
        function dockSubPanel(attr) {
            ...
            var subPanel = subPanels[attr.origin];
