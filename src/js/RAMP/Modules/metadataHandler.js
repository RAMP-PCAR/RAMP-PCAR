            ...
            if (!node.hasClass('selected-row')) {
                ...
                if (layerConfig.layerInfo) {
                    if (layerConfig.legend) {
                        ...
                        tmpl.cache = {};
                        tmpl.templates = JSON.parse(TmplHelper.stringifyTemplate(filterWmsMetaTemplate));
                        ...
                        topic.publish(EventManager.GUI.SUBPANEL_OPEN, {
                            content: '<p>' + i18n.t('filterManager.metadataNotFound') +
                                '</p><b>Service End Point URL</b><br><a href="' + layerConfig.url +
                    ...
                    var metadataError = function () {
                        topic.publish(EventManager.GUI.SUBPANEL_OPEN, {
                            content: '<p>' + i18n.t('filterManager.metadataNotFound') +
                                ...
                                '</h5><p><a href="' + layerConfig.url + '" tagget="_blank">' +
                                layerConfig.url + '</a></p>',
                            origin: 'metadataHandler',
                            update: true,
                            guid: id,
                        ...
                        UtilMisc.transformXML(metadataUrl, 'assets/metadata/xstyle_default_' + RAMP.locale + '.xsl',
                            function (error, data) {
                                if (error) {
                                    ...
                                    topic.publish(EventManager.GUI.SUBPANEL_OPEN, {
                                        content: $(data).append('<h5>' + i18n.t('filterManager.serviceEndPointLabel') +
                                            '</h5><p><a href="' + layerConfig.url + '" tagget="_blank">' +
                                            layerConfig.url + '</a></p>'),
                                        origin: 'metadataHandler',
                                        update: true,
